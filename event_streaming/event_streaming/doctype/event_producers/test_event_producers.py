...
from event_streaming.event_streaming.doctype.event_producers.event_producers import pull_from_node
...
class TestEventProducers(FrappeTestCase):
	def setUp(self):
		create_event_producers(producer_url)
	...
	def test_dynamic_link_dependencies_synced(self):
		...
		event_producers = frappe.get_doc("Event Producer", producer_url, for_update=True)
		event_producers.producer_doctypes = []
		event_producers.append("producer_doctypes", {"ref_doctype": "ToDo", "use_same_name": 1})
		event_producers.save()
	...
	def test_naming_configuration(self):
		...
		event_producers = frappe.get_doc("Event Producer", producer_url, for_update=True)
		event_producers.producer_doctypes = []
		event_producers.append("producer_doctypes", {"ref_doctype": "ToDo", "use_same_name": 0})
		event_producers.save()
	...
	def test_conditional_events(self):
		...
		# Add Condition
		event_producers = frappe.get_doc("Event Producer", producer_url)
		note_producer_entry = [x for x in event_producers.producer_doctypes if x.ref_doctype == "Note"][0]
	...
	def test_conditional_events_with_cmd(self):
		...
		note_producer_entry.condition = (
			"cmd: event_streaming.event_streaming.doctype.event_producers.test_event_producers.can_sync_note"
		)
		event_producers.save()
	...
	def test_inner_mapping(self):
		...
		setup_event_producers_for_inner_mapping()
		producer_note = frappe._dict(
...
def setup_event_producers_for_inner_mapping():
	event_producers = frappe.get_doc("Event Producer", producer_url, for_update=True)
	event_producers.producer_doctypes = []
...
def create_event_producers(producer_url):
	if frappe.db.exists("Event Producer", producer_url):
		event_producers = frappe.get_doc("Event Producer", producer_url)
		for entry in event_producers.producer_doctypes:
			entry.unsubscribe = 0
		event_producers.save()
		return
	...
	event_producers = frappe.new_doc("Event Producer")
	event_producers.producer_doctypes = []
	event_producers.producer_url = producer_url
	event_producers.append("producer_doctypes", {"ref_doctype": "ToDo", "use_same_name": 1})
	event_producers.append("producer_doctypes", {"ref_doctype": "Note", "use_same_name": 1})
	event_producers.user = "Administrator"
	event_producers.api_key = api_key
	event_producers.api_secret = api_secret
	event_producers.save()
